...
final class Nodes {
    ...
    /*▼ (3) 固定长度的Node 这些Node本身也是Sink 可以接收元素 ████████████████████████████████████████████████████████████████████████████████┓ */
    ...
    /*▼ (4) 可变长度的Node 这些Node本身也是Sink 可以接收元素 ████████████████████████████████████████████████████████████████████████████████┓ */
     ...
     */
    // 【Node子类】可串联Node的抽象基类
    private abstract static class AbstractConcNode<T, T_NODE extends Node<T>>
        implements Node<T> {
        // 包含左右两个子Node
    ...
    private abstract static class InternalNodeSpliterator<T, S extends Spliterator<T>, N extends Node<T>>
        ...
        public final S trySplit() {
            if(curNode == null || tryAdvanceSpliterator != null) {  // 已经全部遍历完 或者遍历了一部分 则无法分割
            ...
            } else if(curChildIndex < curNode.getChildCount() - 1) {    // 左孩子
        ...
        private abstract static class OfPrimitive<T, T_CONS, T_ARR, T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>, N extends Node.OfPrimitive<T, T_CONS, T_ARR, T_SPLITR, N>>
