...
public class ReentrantReadWriteLock
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            // 获取state
            int c = getState();
            // state二进制值的低16位代表写锁 相当于位运算获取到state中的低16位 代表了写锁的状态
            ...
            if (c != 0) {   // 说明其他线程已经加过了写锁 || 重入锁处理逻辑
                ...
                if (w + exclusiveCount(acquires) > MAX_COUNT) { // 超过最大重入次数 抛异常
                    throw new Error("Maximum lock count exceeded");
                }
                // Reentrant acquire
                setState(c + acquires); // 可重入加锁
                return true;
            }
            if (writerShouldBlock() ||
                !compareAndSetState(c, c + acquires)) { // 如果是非公平锁 尝试CAS加写锁 如果是公平锁 会判断队列中有无等待线程 有的话不会加写锁
                return false;
            }
            setExclusiveOwnerThread(current);   // 设置持有写锁的线程
            return true;
        }
