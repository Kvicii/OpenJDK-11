    ...
    static class ThreadLocalMap {
        ...
        private void set(ThreadLocal<?> key, Object value) {
            ...
            for (Entry e = tab[index]; e != null; e = tab[index = nextIndex(index, len)]) {
                ...
                /*
                 * 如果当前位置未找到匹配的ThreadLocal就一直遍历Entry(由于哈希值存在碰撞问题所以可能初次计算出的哈希值没法用)
                 * 向后遍历的过程中会出现以下情形:
                 * 1. 找到了匹配的ThreadLocal 那么执行上面的if语句并退出
                 * 2. 遇到了一个垃圾值
                 */
                if (k == null) {
                    /*
         ...
         */
        // 哈希值发生冲突时计算下一个哈希值 此处使用线性探测寻址只是简单地将索引增一
        private static int nextIndex(int i, int len) {
            // 如果索引增一后越界 则返回到下标0的地方循环进行
