...
public class Thread implements Runnable {
	...
	private Thread(ThreadGroup g, Runnable target, String name, long stackSize, AccessControlContext acc, boolean inheritThreadLocals) {
		...
		if (g == null) {
			...
			if (g == null) { // 如果没有指定线程组 那么默认的线程组就是父线程的线程组
		...
		// 默认情况下 如果没有指定是否为daemon的话 那么daemon的状态是由父线程决定的
		this.daemon = parent.isDaemon(); // 守护线程

		// 优先级如果没有指定的话 那么就跟父线程的优先级保持一致
	...
	public synchronized void start() {
		 ...
		 * 如果线程一旦执行过一次以后 那么他的 threadStatu s就一定会变为非0的一个状态 如果 threadStatus 是非0的状态 说明他之前已经被执行过了
		 * 如果对一个线程多次执行start方法 会抛出 IllegalThreadStateException 异常
		 */
		if (threadStatus != 0) {
			throw new IllegalThreadStateException();
		}
		 ...
		 */
		// group就是在构造函数中分配的 如果指定了 那么就是创建的那个ThreadGroup 否则的话就是父线程的ThreadGroup
		...
		try {
			// 一旦是start0成功的启动之后 就会去执行我们覆盖掉的那个run方法
			// 或者通过传入的Runnable对象执行那个Runnable对象的run方法
			start0();
			started = true;
		} finally {
			try {
