...
final class BufferMismatch {
    ...
    static int mismatch(CharBuffer a, int aOff, CharBuffer b, int bOff, int length) {
        int i = 0;
        // Ensure only heap or off-heap buffer instances use the vectorized mismatch.
        // If either buffer is a StringCharBuffer (order is null) then the slow path is taken
        if(length > 3 && a.charRegionOrder() == b.charRegionOrder() && a.charRegionOrder() != null && b.charRegionOrder() != null) {
            if(a.get(aOff) != b.get(bOff))
                return 0;
            i = ArraysSupport.vectorizedMismatch(
                a.base(),
                a.address + (aOff << ArraysSupport.LOG2_ARRAY_CHAR_INDEX_SCALE),
                b.base(),
                b.address + (bOff << ArraysSupport.LOG2_ARRAY_CHAR_INDEX_SCALE),
                length,
    ...
    static int mismatch(ShortBuffer a, int aOff, ShortBuffer b, int bOff, int length) {
        ...
        if(length > 3 && a.order() == b.order()) {
            ...
            i = ArraysSupport.vectorizedMismatch(
                a.base(),
                a.address + (aOff << ArraysSupport.LOG2_ARRAY_SHORT_INDEX_SCALE),
                b.base(),
                b.address + (bOff << ArraysSupport.LOG2_ARRAY_SHORT_INDEX_SCALE),
                length,
    ...
    static int mismatch(IntBuffer a, int aOff, IntBuffer b, int bOff, int length) {
        ...
        if(length > 1 && a.order() == b.order()) {
            ...
            i = ArraysSupport.vectorizedMismatch(
                a.base(),
                a.address + (aOff << ArraysSupport.LOG2_ARRAY_INT_INDEX_SCALE),
                b.base(),
    ...
    static int mismatch(FloatBuffer a, int aOff, FloatBuffer b, int bOff, int length) {
        ...
        if(length > 1 && a.order() == b.order()) {
            if(Float.floatToRawIntBits(a.get(aOff)) == Float.floatToRawIntBits(b.get(bOff))) {
                i = ArraysSupport.vectorizedMismatch(
                    a.base(),
                    a.address + (aOff << ArraysSupport.LOG2_ARRAY_FLOAT_INDEX_SCALE),
                    b.base(),
                    b.address + (bOff << ArraysSupport.LOG2_ARRAY_FLOAT_INDEX_SCALE),
                    length,
    ...
    static int mismatch(LongBuffer a, int aOff, LongBuffer b, int bOff, int length) {
        ...
        if(length > 0 && a.order() == b.order()) {
            ...
            i = ArraysSupport.vectorizedMismatch(
                a.base(),
                a.address + (aOff << ArraysSupport.LOG2_ARRAY_LONG_INDEX_SCALE),
                b.base(),
                b.address + (bOff << ArraysSupport.LOG2_ARRAY_LONG_INDEX_SCALE),
                length,
    ...
    static int mismatch(DoubleBuffer a, int aOff, DoubleBuffer b, int bOff, int length) {
        ...
        if(length > 0 && a.order() == b.order()) {
            if(Double.doubleToRawLongBits(a.get(aOff)) == Double.doubleToRawLongBits(b.get(bOff))) {
                i = ArraysSupport.vectorizedMismatch(
                    a.base(),
                    a.address + (aOff << ArraysSupport.LOG2_ARRAY_DOUBLE_INDEX_SCALE),
                    b.base(),
                    b.address + (bOff << ArraysSupport.LOG2_ARRAY_DOUBLE_INDEX_SCALE),
                    length,
                    ArraysSupport.LOG2_ARRAY_DOUBLE_INDEX_SCALE);
            }
            // Mismatched
            if(i >= 0) {
                // Check if mismatch is not associated with two NaN values; and is not associated with +0 and -0
